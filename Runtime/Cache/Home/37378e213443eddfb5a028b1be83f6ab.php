					...
					<div class="right">
						...
						<div class="r_cnt">
							...
							<?php if(!empty($infos)): if(is_array($infos)): $i = 0; $__LIST__ = $infos;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$info): $mod = ($i % 2 );++$i;?><div class="inform">
								<div class="lt"><?php echo ($info['contact_name']); ?>&nbsp;&nbsp;<?php echo ($info['phone']); ?>&nbsp;&nbsp;<?php echo ($info['address_info']); ?>&nbsp</div>
								<div class="rt">
									<a href="<?php echo U('/member/edit_contact', array('contact_id' => $info['contact_id']));?>" >编辑</a>
									<a href="javascript:void(0);" onclick="delete_(<?php echo ($info['contact_id']); ?>);" >删除</a>
								</div>
							...
							<div class="person">
								<a href="<?php echo U('/member/add_contact');?>">
...
function delete_(contact_id_)
{
	if (confirm("确认删除该联系人吗?")) {
		location.href = "<?php echo C(WWW_GK_COM);?>/member/delete_contact?contact_id="+contact_id_;
    }
}
