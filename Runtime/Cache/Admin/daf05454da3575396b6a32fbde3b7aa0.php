...
<ul class="type_list clear">
    <li class="current"><a>未确认订单</a></li>
</ul>
</div>


<div class="margin10px searchbox">
    <form action="<?php echo U('/order/unconfirmed');?>" method="get">
			...
			<tbody>
				<?php if(!empty($infos)): if(is_array($infos)): $i = 0; $__LIST__ = $infos;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$info): $mod = ($i % 2 );++$i;?><tr>
							...
							<td>
								...
								<?php if($info['pay_status'] == '2'): ?><a title="确认订单" class="btn" href="<?php echo U('order/edit',array('order_id'=>$info['order_id']));?>"  >确认订单</a>
								<?php else: ?>
								<a class="del ajax-get btn" title="取消" url="<?php echo U('order/ajax_delete_order');?>" del_id="<?php echo ($info['order_id']); ?>">取消</a><?php endif; ?>
							</td>
