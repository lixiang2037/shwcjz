...
<ul class="type_list clear">
    ...
    <li class="current"><a href="<?php echo U('kit/add');?>">添加试剂盒</a></li>
</ul>
</div>
<div class="margin10px">
    <form name="form1" method="post" action="<?php echo U('kit/add');?>" onsubmit="return check();">
        <table class="mytable">
            <tr>
                <th colspan="2">添加试剂盒</th>
            </tr>
            <tr>
                <td width="100" class="tdf">名称：</td>
				<td> 
				    <input type="text" id="kit_name" name="kit_name" size="40" /><span></span>
                </td>
            </tr>
            ...
            <tr>
				...
				<td> 
				     <?php if(is_array($search_option["status"])): $i = 0; $__LIST__ = $search_option["status"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$d): $mod = ($i % 2 );++$i;?><input type="radio" name="status" value="<?php echo ($d['id']); ?>" <?php if($d['id'] == $d['default']): ?>checked="checked"<?php endif; ?> />&nbsp;<?php echo ($d["val"]); ?>&nbsp;<?php endforeach; endif; else: echo "" ;endif; ?>
                </td>
            </tr>
