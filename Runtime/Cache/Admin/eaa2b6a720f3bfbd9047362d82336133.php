	...
	<table class="mytable">
			...
			<tbody>
				<?php if(!empty($infos)): if(is_array($infos)): $i = 0; $__LIST__ = $infos;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$info): $mod = ($i % 2 );++$i;?><tr>
							...
							<td>
								<a title="打印报告" class="btn" href="<?php echo U('sample/print_report',array('sample_id'=>$info['sample_id']));?>" target="_blank">打印报告</a>
								<a title="寄送报告" class="btn" href="<?php echo U('sample/send_report',array('sample_id'=>$info['sample_id']));?>" >寄送报告</a>
							</td>
